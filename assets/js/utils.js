...
/* Umami custom-event helper. Pageviews are auto-tracked by the script above; this fires named
   events (register, ai_lesson_open, …). Never let analytics break the UI — guard + swallow. */
function track(name, data){ try { if (window.umami) window.umami.track(name, data); } catch(_){} }
